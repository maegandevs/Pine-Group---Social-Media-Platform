...
def edit_user_profile(root, user_email):
    ...
    tk.Label(main_frame, text="Edit Profile", font=("Arial", 16, "bold")).grid(row=0, column=0, columnspan=2, pady=10)
    ...
    def save_profile_changes():
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("UPDATE users SET name = ?, bio = ? WHERE email = ?", (new_name, new_bio, user_email))
            conn.commit()
    ...
    tk.Button(main_frame, text="Save Changes", width=20, command=save_profile_changes).grid(row=3, column=0, columnspan=2, pady=10)
    tk.Button(main_frame, text="Cancel", width=20, command=lambda: show_user_dashboard(root, user_email)).grid(row=4, column=0, columnspan=2, pady=5)
